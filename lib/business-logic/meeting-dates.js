...
async function getChangesSinceLastMeeting(accountManager, sinceDate = null) {
    ...
    try {
        ...
        const result = await db.query(query, [accountManager, since]);
...
async function getWeeklySummary(accountManager, weekDate = null) {
    ...
    try {
        const query = `
            ...
            AND week_starting_date = ?
        `;
        
        const result = await db.query(query, [accountManager, targetWeek]);
        ...
        if (result.rows.length === 0) {
            ...
            const summaryResult = await db.query(query, [accountManager, targetWeek]);
